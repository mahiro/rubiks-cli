...
namespace rubiks {
    ...
    istream &operator>>(istream &in, Rotation &rotation) {
        ...
        switch (word[0]) {
            ...
            case 'z':
                ...
                break;
            default:
                rotation = Rotation(); // TODO: handle error?
        }

        if (word.length() >= 2) {
            switch (word[1]) {
                case '\'':
                    rotation.turn = ReverseQuarterTurn;
                    break;
                case '2':
                    rotation.turn = HalfTurn;
                    break;
                default:
                    rotation.turn = NullTurn; // TODO: handle error?
            }
        } else {
    ...
    ostream &operator<<(ostream &out, const Procedure &procedure) {
        ...
        for (Procedure::const_iterator it = procedure.begin(); it != procedure.end(); ++it) {
            if (first) {
                first = false;
            } else {
                out << ' ';
    ...
    void reverse(const Procedure &source, Procedure &destination) {
        for (Procedure::const_reverse_iterator it = source.rbegin(); it != source.rend(); ++it) {
            destination.push_back(it->reverse());
        }
    }

    void mirror(const Procedure &source, Procedure &destination) {
        for (Procedure::const_iterator it = source.begin(); it != source.end(); ++it) {
